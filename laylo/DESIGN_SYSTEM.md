# Laylo Design System — Black + Bumblebee Gold
...
| Mobile — Home header | Always visible                      | "Ask Laylo" pill in header.                                      |
| Mobile — Bills/Tasks empty state | Always visible            | Secondary "Ask Laylo to add something" link below primary CTA.   |
...
| Generic empty | "Ask Laylo to add something"  |

### 9.3 Dashboard hero "Ask Laylo" input
...
Header chip: pill 36 px tall, gold border 1 px @ 50%, sparkle icon + "Ask Laylo" label. Tap → opens bottom sheet (`dur-expansive`, `Easing.out(Easing.cubic)`).
...
- **Hero:** "Ask Laylo" input bar (§9.3) full width, top of page. 32 px below greeting.
...
- **Empty state:** centred Bee mascot + Brief §6 copy + primary "Add" button + secondary "Ask Laylo" link.
